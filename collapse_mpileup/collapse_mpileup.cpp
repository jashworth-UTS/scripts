...
public:
	...
	void add( std::string const & str ) {
		size_t lastpos(str.find(pattern_));
//		std::cerr << pattern_ << ": " << str << " " << lastpos << std::endl;
		while(lastpos != string::npos){
			lastpos = str.find(pattern_,lastpos+1);
			++sum_;
//			std::cerr << lastpos << " " << sum_ << std::endl;
		}
	}
