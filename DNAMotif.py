...
#		print 'padded right by %i: %s' %(pad, self.consensus())
	...
	def make_counts_matrix(self,sequences=[]):
		...
		for s in sequences:
			#print s
			l = len(s)
			if l != self.width: raise RuntimeError('length mismatch for seq %s' %s)
			for i in range(l):
				base = s[i].upper()
				if not base in self.bases and not base in self.degen: raise RuntimeError('unknown base: %s' %base)
				if base in self.degen:
					bases = self.degen[base]
					for b in bases:
						self.matrix['counts'][i][b] += 1
				else:
...
#					print '%i,%s,diff %g' %(i,b,v2-v1)
	...
	def output_default(self,type='probs',name=None):
		...
		out.append('key%s%s' %(self.sep,self.sep.join(self.bases)))
		for i in range(self.width):
			line = str(i)
			for base in self.bases:
				value = ''
	...
	def output_TRANSFAC(self,name):
		...
		out.append('PO%s%s' %(self.sep,''.join(self.bases)))
		for i in range(self.width):
			line = str(i+1)
			for base in self.bases:
...
#				print string.join([s.seq for s in seqs],'\n')
			...
			for seq in seqs:
				...
				elif l != seqlen: raise RuntimeError('length mismatch for seq %s l %i seqlen %i' %(str(seq),l,seqlen))
