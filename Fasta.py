...
class Fasta:
	...
	def promseq(self,start,end,antisense,upstream=-350,downstream=50):
		...
		pseq = self.subseq(promstart, promend)
		if antisense: pseq.rvscomp()
		return(pseq)
...
class FastaSeqs:
	...
	op.add_option('--promoter',action='store_true')
	op.add_option('--promoter_start',type='int',default='-350')
	op.add_option('--promoter_end',type='int',default='150')
...
#		mn,mx,md = self.seqstats()
	...
	def process(self,opt):
		if opt.promoter:
			...
			promseqs = self.promoterseqs(windows,opt.promoter_start,opt.promoter_end)
			# deep copy/replace
			self.seqs = promseqs.seqs
			self.order = promseqs.order
		elif opt.subseqs:
			windows=SeqWindows()
			windows.load(opt.subseqs)
	...
	def rvscomp(self):
		for seq in self.seqs.values(): seq.rvscomp()
	...
	def promoterseqs(self,windows,promoter_start,promoter_end):
		...
		for seq,windows in windows.items():
			...
			for window in windows:
				...
				promseqs.seqs[promseq.name] = promseq
				promseqs.order.append(promseq.name)
		return promseqs
